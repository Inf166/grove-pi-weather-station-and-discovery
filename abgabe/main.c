...
/* main-Function */
  int main() {
    ...
    snprintf(connectedClients[0]->ipdesclient, sizeof(connectedClients[0]->ipdesclient),
             (char *) inet_ntoa(server_address.sin_addr));
    connectedClients[0]->isNotEmpty = 1;
    time_t now;
    now = time(0);
//    connectedClients[0]->connectionEstablished = ctime(&now);
    snprintf(connectedClients[0]->connectionEstablished, sizeof(connectedClients[0]->connectionEstablished),ctime(&now));
    nextFreeEntryPlace++;
                                ...
                                if (strcmp(args[0], "GET") == 0) {
                                    ...
                                    while (connectedClients[i]->isNotEmpty) {
                                        printf("<%s> [Connect since: %s]\n", connectedClients[i]->ipdesclient,
                                               connectedClients[i]->connectionEstablished);
                                ...
                                if (semID >= 0) {
                                    ...
                                    while (tag > 0) {
                                        ...
                                        tag = msgrcv(feeder, &nachricht, sizeof(&nachricht),
                                                     sizeof(connectedClients[counter]), 0);
                                        if (tag < 0) {
...
//                        connectedClients[nextFreeEntryPlace]->ipdesclient = inet_ntoa(client_addr.sin_addr);
                        snprintf(connectedClients[nextFreeEntryPlace]->ipdesclient, sizeof(connectedClients[nextFreeEntryPlace]->ipdesclient),
                                 (char *) inet_ntoa(client_addr.sin_addr));
                        time_t now;
                        now = time(0);
//                        connectedClients[nextFreeEntryPlace]->connectionEstablished = ctime(&now);
                        snprintf(connectedClients[nextFreeEntryPlace]->connectionEstablished, sizeof(connectedClients[nextFreeEntryPlace]->connectionEstablished), ctime(&now));
                            ...
                            while (1) {
                                if (semID >= 0) {
                                    ...
                                    while (tag > 0) {
                                        ...
                                        tag = msgrcv(feeder, &nachricht, sizeof(&nachricht),
                                                     sizeof(connectedClients[cougar]), 0);
                                        if (tag < 0) {
                                            perror(strerror(errno));
                                            printf("msgrcv failed, rc=%d\n", tag);
                                            exit(1);
                            ...
                            while (1) {
                                if (semID >= 0) {
                                    ...
                                    if (informationen[0] == "Temperatur") {
                                        connectedClients[cougar]->meineSensorwerte.akTemp = atof(informationen[1]);
                                        if (connectedClients[cougar]->meineSensorwerte.minTemp >
                                            atof(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.minTemp = atof(
                                                    informationen[1]);
                                        else if (connectedClients[cougar]->meineSensorwerte.maxTemp <
                                                 atof(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.maxTemp = atof(
                                                    informationen[1]);
                                    ...
                                    if (informationen[0] == "Humidity") {
                                        connectedClients[cougar]->meineSensorwerte.akHum = atof(informationen[1]);
                                        if (connectedClients[cougar]->meineSensorwerte.minHum > atof(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.minHum = atof(informationen[1]);
                                        else if (connectedClients[cougar]->meineSensorwerte.maxHum <
                                                 atof(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.maxHum = atof(informationen[1]);
                                    }
                                    ...
                                    if (informationen[0] == "Sound") {
                                        connectedClients[cougar]->meineSensorwerte.akDB = atof(informationen[1]);
                                        if (connectedClients[cougar]->meineSensorwerte.minDB > atof(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.minDB = atof(informationen[1]);
                                        else if (connectedClients[cougar]->meineSensorwerte.maxDB <
                                                 atof(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.maxDB = atof(informationen[1]);
                                    }
                                    ...
                                    if (informationen[0] == "Moisture") {
                                        connectedClients[cougar]->meineSensorwerte.akWater = atoi(informationen[1]);
                                        if (connectedClients[cougar]->meineSensorwerte.minWater >
                                            atoi(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.minWater = atoi(
                                                    informationen[1]);
                                        else if (connectedClients[cougar]->meineSensorwerte.maxWater <
                                                 atoi(informationen[1]))
                                            connectedClients[cougar]->meineSensorwerte.maxWater = atoi(
                                                    informationen[1]);
                                    }
                                    ...
                                    if (informationen[0] == "Motiondetected") {
                                        connectedClients[cougar]->meineSensorwerte.motion = 1;
                                    }
                                    ...
                                    if (informationen[0] == "Colisiondetected") {
                                        connectedClients[cougar]->meineSensorwerte.colision = atoi(informationen[1]);
                                    }
